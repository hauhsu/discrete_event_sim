...
class Person
{
  public:
    ...
    unsigned get_serve_time() const {return m_serve_time;}

    void set_arrival_time(unsigned time) {
      std::cout << "set arrival time: " << time << std::endl;
      m_arrival_time = time;}
    void set_leave_time(unsigned time) {
      std::cout << "set leave time: " << time << std::endl; m_leave_time= time;}

    unsigned get_arrival_time() const {return m_arrival_time;}
    unsigned get_leave_time() const {return m_leave_time;}

    unsigned get_system_time() const {return m_leave_time - m_arrival_time;}
  private:
    unsigned m_id;
...
public:
  ...
  void gen_arrival_event(const int num_event)
  {
    for (int i = 0; i < num_event; ++i) {
      unsigned arrival_time = m_rand_arrival_time.rand_exp_distribuiton(5) * 100 + current_time();
